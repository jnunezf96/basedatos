#!/usr/bin/env python3
"""Clean visible inline bracket residue in 1580 Sahagun/Maynez prose.

This pass protects source-context labels in ``<small>...</small>`` and bold
Nahuatl spans. It only removes square-bracket notation from remaining public
prose, where the brackets are editorial completion markers rather than useful
display structure.
"""
...
PROPOSALS_PATH = Path("resources/sahagun_maynez_1580_inline_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/sahagun_maynez_1580_inline_bracket_summary.json")
SOURCE = "1580 Sahagún/Máynez"
RAW_FIELD = "Comentario_raw_1580_sahagun_maynez"
MARKER = "visible_1580_sahagun_maynez_inline_bracket_cleanup_2026_06_29"
QA_KEY = "qa_1580_sahagun_maynez_inline_bracket_cleanup"
...
SPECIAL_REPLACEMENTS = {
    "comeran [ian]": "comerían",
}
CROSS_TAG_REPLACEMENTS = {
    "[diosa de los <b>xilotes</b>]": "diosa de los <b>xilotes</b>",
}
SPECIAL_ITEMS = sorted(SPECIAL_REPLACEMENTS.items(), key=lambda item: len(item[0]), reverse=True)
...
def normalize_piece(piece: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    piece = BRACKET_RE.sub(replace_match, piece)
