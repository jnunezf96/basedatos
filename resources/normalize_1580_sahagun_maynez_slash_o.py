#!/usr/bin/env python3
"""Normalize slash-marked Spanish o alternatives in 1580 Sahagun/Maynez."""
...
PROPOSALS_PATH = Path("resources/sahagun_maynez_1580_slash_o_proposals.tsv")
SUMMARY_PATH = Path("resources/sahagun_maynez_1580_slash_o_summary.json")
SOURCE = "1580 Sahagún/Máynez"
RAW_FIELD = "Comentario_raw_1580_sahagun_maynez_slash_o"
MARKER = "visible_spanish_1580_sahagun_maynez_slash_o_2026_06_29"
QA_KEY = "qa_1580_sahagun_maynez_slash_o"

COMMENTARY_FIELDS = ["Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
BOLD_RE = re.compile(r"(<b\b[^>]*>.*?</b>)", re.I | re.S)
BR_RE = re.compile(r"<br\s*/?>", re.I)
TAG_RE = re.compile(r"<[^>]+>")
SLASH_BEFORE_O_RE = re.compile(r"\s*/o\b")
SLASH_AFTER_O_RE = re.compile(r"\bo/(?=\S)")
...
def normalize_piece(piece: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    def before_o(match: re.Match[str]) -> str:
        old = match.group(0)
        new = " o"
        changes.append((old, new))
        return new

    def after_o(match: re.Match[str]) -> str:
        old = match.group(0)
        new = "o "
        changes.append((old, new))
        return new

    piece = SLASH_BEFORE_O_RE.sub(before_o, piece)
    piece = SLASH_AFTER_O_RE.sub(after_o, piece)
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "normalized_slash_marked_spanish_o_alternatives_outside_bold",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
