#!/usr/bin/env python3
"""Clean remaining visible square-bracket residue in 1629 Alarcon."""
...
PROPOSALS_PATH = Path("resources/alarcon_1629_remaining_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/alarcon_1629_remaining_bracket_summary.json")
SOURCE = "1629 Alarcón"
RAW_FIELD = "Comentario_raw_1629_alarcon_remaining_brackets"
MARKER = "visible_1629_alarcon_remaining_bracket_cleanup_2026_06_29"
QA_KEY = "qa_1629_alarcon_remaining_bracket_cleanup"
...
PAGE_NOTE_RE = re.compile(r"^(?:en\s+)?\d+(?:\s*[-–]\s*\d+)?$", re.I)
SIC_RE = re.compile(r"^sic(?:[, ].*)?$", re.I)
...
def replacement_for_match(piece: str, match: re.Match[str]) -> str:
    ...
    if PAGE_NOTE_RE.fullmatch(inner):
        if before == "(":
            return inner
        return with_edge_spacing(piece, match, f"({inner})")

    if SIC_RE.fullmatch(inner):
        return with_edge_spacing(piece, match, f"({inner})")
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "cleaned_remaining_visible_square_brackets_outside_small_and_bold",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
