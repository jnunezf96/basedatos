#!/usr/bin/env python3
"""Normalize glued old-Spanish de-forms in 1571 Molina 2 commentary."""
...
PROPOSALS_PATH = Path("resources/molina2_1571_oldspanish_proposals.tsv")
SUMMARY_PATH = Path("resources/molina2_1571_oldspanish_summary.json")
SOURCE = "1571 Molina 2"
RAW_FIELD = "Comentario_raw_1571_molina2_oldspanish"
MARKER = "visible_spanish_1571_molina2_oldwriting_2026_06_29"
QA_KEY = "qa_1571_molina2_oldspanish"
...
REPLACEMENTS: list[tuple[re.Pattern[str], str]] = [
    (re.compile(r"(?<![\w-])hazerseme(?![\w-])", re.I), "hacérseme"),
    (re.compile(r"(?<![\w-])hazerlos(?![\w-])", re.I), "hacerlos"),
    (re.compile(r"(?<![\w-])peruertimiento(?![\w-])", re.I), "pervertimiento"),
    (re.compile(r"(?<![\w-])bexigas(?![\w-])", re.I), "vejigas"),
    (re.compile(r"(?<![\w-])dezidor(?![\w-])", re.I), "decidor"),
    (re.compile(r"(?<![\w-])rezio(?![\w-])", re.I), "recio"),
    (re.compile(r"(?<![\w-])reyr(?![\w-])", re.I), "reír"),
    (re.compile(r"(?<![\w-])enlos(?![\w-])", re.I), "en los"),
    (re.compile(r"(?<![\w-])delas(?![\w-])", re.I), "de las"),
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "normalized_glued_old_spanish_de_forms_in_molina2_commentary",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
