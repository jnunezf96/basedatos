#!/usr/bin/env python3
"""Clean visible square-bracket note residue in 1547 Olmos_G commentary."""
...
PROPOSALS_PATH = Path("resources/olmos_g_1547_visible_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/olmos_g_1547_visible_bracket_summary.json")
SOURCE = "1547 Olmos_G"
RAW_FIELD = "Comentario_raw_1547_olmos_g_visible_brackets"
MARKER = "visible_1547_olmos_g_bracket_cleanup_2026_06_29"
QA_KEY = "qa_1547_olmos_g_visible_bracket_cleanup"

COMMENTARY_FIELDS = ["Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
BRACKET_WITH_SIGLA_RE = re.compile(r"\[([^\[\]]*)\](MN|BN)?")
BR_RE = re.compile(r"<br\s*/?>", re.I)
TAG_RE = re.compile(r"<[^>]+>")

POST_REPAIRS = {
    "§nitetlani <b>, ganar en juego, <b>onitetlan</b> MN</b>": "§nitetlani, ganar en juego, <b>onitetlan</b> MN",
    "§vel <b>itic coyonqui MN</b>": "§vel <b>itic coyonqui</b> MN",
    "§ocoçauix, <b>Molina; Paredes Sim.</b>": "§ocoçauix, <b>Molina; Paredes</b> Sim.",
}
...
def tidy_visible_note_spacing(text: str) -> str:
    text = re.sub(r"\s{2,}", " ", text)
    text = re.sub(r"\s+([,.;:])", r"\1", text)
    text = re.sub(r"([,.;:])(?=\S)", r"\1 ", text)
    text = re.sub(r"\s+</b>", "</b>", text)
    text = re.sub(r"<b>\s+", "<b>", text)
    for old, new in POST_REPAIRS.items():
...
def normalize_value(value: object) -> tuple[str, list[tuple[str, str]]]:
    ...
    def repl(match: re.Match[str]) -> str:
        ...
        sigla = match.group(2) or ""
        new = " ".join(part for part in [inner, sigla] if part).strip()
        changes.append((old, new))
        return new

    new_text = BRACKET_WITH_SIGLA_RE.sub(repl, text)
    if new_text.startswith("[") and "]" not in new_text:
        changes.append(("[", ""))
        new_text = new_text[1:].lstrip()
    if changes:
        new_text = tidy_visible_note_spacing(new_text)
    if new_text == text:
        return text, []
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "cleaned_visible_square_bracket_note_residue",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
