#!/usr/bin/env python3
"""Remove square brackets around numeric page refs inside 1629 Alarcon bold examples."""
...
PROPOSALS_PATH = Path("resources/alarcon_1629_bold_page_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/alarcon_1629_bold_page_bracket_summary.json")
SOURCE = "1629 Alarcón"
RAW_FIELD = "Comentario_raw_1629_alarcon_bold_page_brackets"
MARKER = "visible_1629_alarcon_bold_page_bracket_cleanup_2026_06_30"
QA_KEY = "qa_1629_alarcon_bold_page_bracket_cleanup"

COMMENTARY_FIELDS = ["Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
BOLD_RE = re.compile(r"(<b\b[^>]*>)(.*?)(</b>)", re.I | re.S)
PAGE_BRACKET_RE = re.compile(r"\[(\d{1,4})\]")
...
def normalize_bold_piece(piece: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    piece = PAGE_BRACKET_RE.sub(repl, piece)
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "removed_square_brackets_around_numeric_page_refs_inside_bold_alarcon_examples",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
