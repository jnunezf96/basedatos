#!/usr/bin/env python3
"""Normalize visible cedilla residue in 1580 Sahagun/Maynez commentary."""
...
PROPOSALS_PATH = Path("resources/sahagun_maynez_1580_cedilla_proposals.tsv")
SUMMARY_PATH = Path("resources/sahagun_maynez_1580_cedilla_summary.json")
SOURCE = "1580 Sahagún/Máynez"
RAW_FIELD = "Comentario_raw_1580_sahagun_maynez_cedilla"
MARKER = "visible_1580_sahagun_maynez_cedilla_oldwriting_2026_06_29"
QA_KEY = "qa_1580_sahagun_maynez_cedilla_oldwriting"
...
EXACT_REPLACEMENTS: dict[str, str] = {
    "Tlaçulteoutl": "Tlazolteotl",
    "Tlaçulteutl": "Tlazolteotl",
    "tlaçulteutl": "tlazolteotl",
    "neçaoalpilli": "nezahualpilli",
    "netlalocaçaoalistli": "netlalocazahualiztli",
    "netlalocaçaoaliztli": "netlalocazahualiztli",
    "çaquan": "zacuan",
}
...
def cedilla_to_cz(token: str) -> str:
    ...
    for index, char in enumerate(chars):
        ...
        replacement = "c" if next_char in CEDILLA_EI else "z"
        if char == "Ç" and index == 0:
...
def normalize_token(token: str) -> str:
    exact = EXACT_REPLACEMENTS.get(token)
    if exact:
        return exact
    return cedilla_to_cz(token).replace("quj", "qui").replace("Quj", "Qui").replace("QUJ", "QUI")
...
def normalize_piece(piece: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    def repl(match: re.Match[str]) -> str:
        old = match.group(0)
        if "ç" not in old and "Ç" not in old:
            return old
        new = normalize_token(old)
        if new != old:
            changes.append((old, new))
        return new

    return WORD_RE.sub(repl, piece), changes
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "normalized_visible_cedilla_residue_outside_bold",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
