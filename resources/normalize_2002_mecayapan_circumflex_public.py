#!/usr/bin/env python3
"""Strip Mecayapan source circumflex vowels in public display fields."""
...
PROPOSALS_PATH = Path("resources/2002_mecayapan_circumflex_public_proposals.tsv")
SUMMARY_PATH = Path("resources/2002_mecayapan_circumflex_public_summary.json")
MARKER = "visible_2002_mecayapan_circumflex_public_2026_06_30"
QA_KEY = "qa_2002_mecayapan_circumflex_public"

TARGET_SOURCE = "2002 Mecayapan"
PUBLIC_FIELDS = ["Traducción", "Traducción (es)", "Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
RAW_FIELD_SUFFIX = "_raw_2002_mecayapan_circumflex_public"

BR_RE = re.compile(r"<br\s*/?>", re.I)
TAG_RE = re.compile(r"<[^>]+>")
CIRCUMFLEX_RE = re.compile("[ÂÊÎÔÛâêîôû]")
CIRCUMFLEX_MAP = str.maketrans({
    "Â": "A",
    "Ê": "E",
    "Î": "I",
    "Ô": "O",
...
def normalize_text(value: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    def repl(match: re.Match[str]) -> str:
        old = match.group(0)
        new = old.translate(CIRCUMFLEX_MAP)
        changes.append((old, new))
        return new

    return CIRCUMFLEX_RE.sub(repl, value), changes
...
def main() -> int:
    ...
    for row in rows:
        ...
        for field in PUBLIC_FIELDS:
            value = row.get(field, "")
            if not isinstance(value, str) or not CIRCUMFLEX_RE.search(value):
                continue
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "stripped_2002_mecayapan_source_circumflex_vowels_in_public_display_fields",
                    "marker": MARKER,
