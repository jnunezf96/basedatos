#!/usr/bin/env python3
"""Normalize selected Sahagún/Máynez h/v old-spelling forms in commentary."""
...
PROPOSALS_PATH = Path("resources/commentary_maynez_h_v_proposals.tsv")
SUMMARY_PATH = Path("resources/commentary_maynez_h_v_summary.json")
MARKER = "visible_commentary_maynez_h_v_2026_06_30"
QA_KEY = "qa_commentary_maynez_h_v"

TARGET_SOURCE = "1580 Sahagún/Máynez"
DISPLAY_FIELDS = ["Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
RAW_FIELD_BY_FIELD = {
    "Comentario": "Comentario_public_raw_maynez_h_v",
    "Comentario (es)": "Comentario_es_raw_maynez_h_v",
    "Comentario_wimmer_plus_html": "Comentario_wimmer_plus_html_raw_maynez_h_v",
}
...
EXACT_REPLACEMENTS = {
    "hiviquentzin": "ihuiquentzin",
    "hivitica": "ihuitica",
    "hatlapalli": "atlapalli",
    "haxalli": "axalli",
    "chiquivitl": "chiquihuitl",
    "chiquivites": "chiquihuites",
    "avevetl": "ahuehuetl",
    "avexocaquilitl": "ahuexocaquilitl",
    "covixcatl": "cohuixcatl",
    "covixco": "cohuixco",
    "coyovacan": "coyohuacan",
    "quetzaliacavitztli": "quetzaliacahuitztli",
    "avitzotl": "ahuitzotl",
    "vaxquavitl": "huaxquahuitl",
}
...
def replacement_for_word(word: str) -> str | None:
    folded = word.lower()
    if folded in EXACT_REPLACEMENTS:
        return case_like(word, EXACT_REPLACEMENTS[folded])
    if "quavitl" in folded:
        return re.sub("quavitl", "quahuitl", word, flags=re.I)
    return None
...
def main() -> int:
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa[QA_KEY] = {
                "action": "normalized_maynez_h_v_old_spelling_in_commentary",
                "marker": MARKER,
