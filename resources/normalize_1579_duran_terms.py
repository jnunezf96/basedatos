#!/usr/bin/env python3
"""Normalize safe visible Nahuatl term spelling in 1579 Duran.

Duran commentary is Spanish chronicle prose with bold Nahuatl terms. This pass
rewrites only bold spans, preserving the public commentary in a raw field before
display changes. It is intentionally narrow: the current automatic map is only
for source-attested bold terms whose row headword already resolves the spelling.
"""
...
PROPOSALS_PATH = Path("resources/duran_1579_term_normalization_proposals.tsv")
REVIEW_PATH = Path("resources/duran_1579_term_normalization_review.tsv")
SUMMARY_PATH = Path("resources/duran_1579_term_normalization_summary.json")
SOURCE = "1579 Durán"
RAW_FIELD = "Comentario_raw_1579_duran"
SOURCE_TERM_MAP_MARKER = "visible_bold_nahuatl_1579_source_term_map_2026_06_29"
...
REVIEW_PATTERNS: list[tuple[str, re.Pattern[str], str]] = [
    (
        ...
        "review cedilla in bold Nahuatl term",
    ),
    (
        "qu_before_a_o",
        re.compile(r"\bqu[aoāōáóàòâô][\wÁÉÍÓÚÜÑáéíóúüñĀĒĪŌāēīōÂÊÎÔâêîôÀÈÌÒÙàèìòù\[\]]*", re.I),
        "review qu before a/o in bold Nahuatl term",
    ),
    (
        "v_likely_u",
        re.compile(r"\b[vV][aeiouāēīōáéíóúàèìòùâêîô]\w*|\w*[aeiouāēīōáéíóúàèìòùâêîô][vV][aeiouāēīōáéíóúàèìòùâêîô]\w*"),
        "review v/u historical spelling in bold Nahuatl term",
    ),
    (
        "initial_h_before_vowel",
        re.compile(r"\bh[aeioāēīōáéíóàèìòâêîô]\w*", re.I),
        "review initial h before vowel in bold Nahuatl term",
    ),
    (
        "nasal_or_diaeresis",
        re.compile(r"\b[\wãẽĩõÃẼĨÕäëïöüÄËÏÖÜ]*[ãẽĩõÃẼĨÕäëïöüÄËÏÖÜ][\wãẽĩõÃẼĨÕäëïöüÄËÏÖÜ]*\b"),
        "review nasal tilde or diaeresis convention in bold Nahuatl term",
    ),
]
    ...
    for row in rows:
        ...
        if row_changes:
            ...
            if args.apply:
                ...
                qa = {
                    **qa,
                    "qa_1579_duran_visible_bold_oldspell": {
                        "action": "normalized_visible_old_spellings_inside_bold_nahuatl_terms",
