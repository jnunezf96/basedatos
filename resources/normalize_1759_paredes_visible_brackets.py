#!/usr/bin/env python3
"""Clean remaining visible square-bracket notes in 1759 Paredes commentary."""
...
PROPOSALS_PATH = Path("resources/paredes_1759_visible_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/paredes_1759_visible_bracket_summary.json")
SOURCE = "1759 Paredes"
RAW_FIELD = "Comentario_raw_1759_paredes_visible_brackets"
MARKER = "visible_1759_paredes_bracket_cleanup_2026_06_29"
QA_KEY = "qa_1759_paredes_visible_bracket_cleanup"
...
def inside_parentheses(piece: str, index: int) -> bool:
    depth = 0
    for char in piece[:index]:
        if char == "(":
            depth += 1
        elif char == ")" and depth:
            depth -= 1
...
def replacement_for_match(piece: str, match: re.Match[str]) -> str:
    ...
    if not inner:
        return ""
    replacement = f"({inner})" if inside_parentheses(piece, match.start()) else inner
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "cleaned_visible_square_bracket_notes_outside_small_and_bold",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
