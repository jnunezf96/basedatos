#!/usr/bin/env python3
"""Remove in-word square brackets in 1551-95 Documentos display text.

This pass only handles embedded restorations such as ``tecpo[yo]tl``. It leaves
standalone editorial notes, folios, citations, [sic], and [rúbrica] untouched.
"""
...
PROPOSALS_PATH = Path("resources/documentos_1551_inword_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/documentos_1551_inword_bracket_summary.json")
SOURCE = "1551-95 Documentos nahuas de la Ciudad de México"
RAW_FIELD = "Comentario_raw_1551_documentos_inword_brackets"
MARKER = "visible_1551_documentos_inword_bracket_cleanup_2026_06_29"
QA_KEY = "qa_1551_documentos_inword_bracket_cleanup"

PUBLIC_FIELDS = ["Traducción", "Traducción (es)", "Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
BRACKET_RE = re.compile(r"\[([A-Za-zÁÉÍÓÚÜÑáéíóúüñĀĒĪŌāēīōÂÊÎÔâêîôÇç]{1,12})\]")
...
def normalize_value(value: object) -> tuple[str, list[tuple[str, str]]]:
    ...
    def repl(match: re.Match[str]) -> str:
        ...
        if not (is_word_char(before) and is_word_char(after)):
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "removed_inword_square_brackets_from_1551_documentos_display_text",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
