#!/usr/bin/env python3
"""Expand visible Spanish bracket abbreviations in 1645 Carochi commentary.

Carochi public commentary mixes bold Nahuatl examples with Spanish glosses and
grammar notes. This pass only expands alphabetic bracket abbreviations outside
bold spans, preserving the raw commentary field before display changes.
"""
...
PROPOSALS_PATH = Path("resources/carochi_1645_spanish_abbreviation_proposals.tsv")
SUMMARY_PATH = Path("resources/carochi_1645_spanish_abbreviation_summary.json")
SOURCE = "1645 Carochi"
RAW_FIELD = "Comentario_raw_1645_carochi"
MARKER = "visible_spanish_1645_carochi_abbreviation_expansion_2026_06_29"
QA_KEY = "qa_1645_carochi_spanish_abbreviation_expansion"

COMMENTARY_FIELDS = ["Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
BOLD_RE = re.compile(r"(<b\b[^>]*>.*?</b>)", re.I | re.S)
BR_RE = re.compile(r"<br\s*/?>", re.I)
TAG_RE = re.compile(r"<[^>]+>")
ALPHA_BRACKET_RE = re.compile(
    r"[A-Za-zÀ-ÖØ-öø-ÿĀ-ž]+(?:\[[A-Za-zÀ-ÖØ-öø-ÿĀ-ž]+\][A-Za-zÀ-ÖØ-öø-ÿĀ-ž]*)+"
)
BRACKET_RE = re.compile(r"\[([A-Za-zÀ-ÖØ-öø-ÿĀ-ž]+)\]")
...
def expand_bracket_token(token: str) -> str:
    return BRACKET_RE.sub(lambda match: match.group(1), token)


def should_preserve_token(token: str) -> bool:
    if token.isupper():
        return True
    bracket_parts = [part.lower() for part in BRACKET_RE.findall(token)]
    if token.endswith("]") and any(part in {"tl", "tli", "itl"} for part in bracket_parts):
        return True
    expanded = expand_bracket_token(token).lower()
    if expanded in {"tlatquitl", "axcaitl", "icniuh", "ititl"}:
        return True
    return False
...
def normalize_piece(piece: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    def repl(match: re.Match[str]) -> str:
        old = match.group(0)
        if should_preserve_token(old):
            return old
        new = expand_bracket_token(old)
        if new != old:
            changes.append((old, new))
        return new

    return ALPHA_BRACKET_RE.sub(repl, piece), changes
