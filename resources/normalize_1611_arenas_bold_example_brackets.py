#!/usr/bin/env python3
"""Remove square brackets around supplied text in 1611 Arenas bold examples."""
...
PROPOSALS_PATH = Path("resources/arenas_1611_bold_example_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/arenas_1611_bold_example_bracket_summary.json")
SOURCE = "1611 Arenas"
RAW_FIELD = "Comentario_raw_1611_arenas_bold_example_brackets"
MARKER = "visible_1611_arenas_bold_example_bracket_cleanup_2026_06_30"
QA_KEY = "qa_1611_arenas_bold_example_bracket_cleanup"

COMMENTARY_FIELDS = ["Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
BOLD_RE = re.compile(r"(<b\b[^>]*>)(.*?)(</b>)", re.I | re.S)
BRACKET_RE = re.compile(r"\[([A-Za-zÁÉÍÓÚÜÑáéíóúüñĀĒĪŌāēīōÀ-ÖØ-öø-ÿ\s]+)\]")
...
def normalize_bold_piece(piece: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    def repl(match: re.Match[str]) -> str:
        old = match.group(0)
        new = re.sub(r"\s+", " ", match.group(1)).strip()
        if not new:
            return old
...
def normalize_value(value: object) -> tuple[str, list[tuple[str, str]]]:
    ...
    def repl(match: re.Match[str]) -> str:
        open_tag, piece, close_tag = match.groups()
        new_piece, piece_changes = normalize_bold_piece(piece)
        changes.extend(piece_changes)
        return f"{open_tag}{new_piece}{close_tag}"
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "removed_square_brackets_around_supplied_text_inside_bold_arenas_examples",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
