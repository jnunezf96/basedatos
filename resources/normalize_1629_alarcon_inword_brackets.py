#!/usr/bin/env python3
"""Remove in-word square brackets in 1629 Alarcon display text.

This pass only handles embedded restorations such as ``co[z]ahuic`` and
``àquenman[]paqui``. Standalone glosses and translation brackets are left alone.
"""
...
PROPOSALS_PATH = Path("resources/alarcon_1629_inword_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/alarcon_1629_inword_bracket_summary.json")
SOURCE = "1629 Alarcón"
RAW_FIELD = "Comentario_raw_1629_alarcon_inword_brackets"
MARKER = "visible_1629_alarcon_inword_bracket_cleanup_2026_06_29"
QA_KEY = "qa_1629_alarcon_inword_bracket_cleanup"

PUBLIC_FIELDS = ["Traducción", "Traducción (es)", "Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
BRACKET_RE = re.compile(r"\[([A-Za-zÁÉÍÓÚÜÑáéíóúüñĀĒĪŌāēīōÂÊÎÔâêîôÇç]{0,12})\]")
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "removed_inword_square_brackets_from_1629_alarcon_display_text",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
