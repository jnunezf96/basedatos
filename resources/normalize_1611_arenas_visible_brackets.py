#!/usr/bin/env python3
"""Clean visible square-bracket gloss residue in 1611 Arenas."""
...
PROPOSALS_PATH = Path("resources/arenas_1611_visible_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/arenas_1611_visible_bracket_summary.json")
SOURCE = "1611 Arenas"
RAW_FIELD = "Comentario_raw_1611_arenas_visible_brackets"
MARKER = "visible_1611_arenas_bracket_cleanup_2026_06_29"
QA_KEY = "qa_1611_arenas_visible_bracket_cleanup"
...
NUMERIC_NOTE_RE = re.compile(r"^\d+(?:\s*[-–]\s*.+)?$", re.U)
...
def replacement_for_match(piece: str, match: re.Match[str]) -> str:
    ...
    if NUMERIC_NOTE_RE.fullmatch(inner):
        replacement = f"({inner})"
    else:
        replacement = inner

    compact_insert = " " not in replacement and len(replacement) <= 4 and (is_word_char(before) or is_word_char(after))
    prefix = ""
    suffix = ""
    if not compact_insert and is_word_char(before) and (is_word_char(replacement[:1]) or replacement.startswith("(")):
        prefix = " "
    if not compact_insert and is_word_char(after) and (is_word_char(replacement[-1:]) or replacement.endswith(")")):
        suffix = " "
