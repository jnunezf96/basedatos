#!/usr/bin/env python3
"""Remove visible archaic-Spanish helper annotations from public fields."""
...
PROPOSALS_PATH = Path("resources/public_archaic_spanish_annotation_proposals.tsv")
SUMMARY_PATH = Path("resources/public_archaic_spanish_annotation_summary.json")
MARKER = "visible_public_archaic_spanish_annotation_cleanup_2026_06_29"
QA_KEY = "qa_public_archaic_spanish_annotation_cleanup"

PUBLIC_FIELDS = ["Traducción", "Traducción (es)", "Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
RAW_FIELD_SUFFIX = "_raw_public_archaic_spanish_annotations"
BR_RE = re.compile(r"<br\s*/?>", re.I)
TAG_RE = re.compile(r"<[^>]+>")

REPLACEMENTS: list[tuple[re.Pattern[str], str]] = [
    (re.compile(r"agora \(arcaico: ahora\)", re.I), "ahora"),
    (re.compile(r"(?<![\w-])ques del\?", re.I), "que es de él?"),
]
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "removed_visible_archaic_spanish_parenthetical_annotations",
                    "marker": MARKER,
