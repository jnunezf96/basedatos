#!/usr/bin/env python3
"""Normalize 1759 Paredes grave-accent residue in public display fields."""
...
PROPOSALS_PATH = Path("resources/paredes_1759_grave_public_proposals.tsv")
SUMMARY_PATH = Path("resources/paredes_1759_grave_public_summary.json")
SOURCE = "1759 Paredes"
MARKER = "visible_1759_paredes_grave_public_2026_06_30"
QA_KEY = "qa_1759_paredes_grave_public"

PUBLIC_FIELDS = ["Traducción", "Traducción (es)", "Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
RAW_FIELD_SUFFIX = "_raw_1759_paredes_grave_public"

BR_RE = re.compile(r"<br\s*/?>", re.I)
TAG_RE = re.compile(r"<[^>]+>")
GRAVE_RE = re.compile("[ÀÈÌÒÙàèìòù]")
WORD_RE = re.compile(r"[A-Za-zÀÈÌÒÙàèìòùĀĒĪŌŪāēīōūÂÊÎÔÛâêîôûÁÉÍÓÚÜÑáéíóúüñÇç]+")
GRAVE_MAP = str.maketrans("ÀÈÌÒÙàèìòù", "AEIOUaeiou")
...
def main() -> int:
    ...
    for row in rows:
        ...
        for field in PUBLIC_FIELDS:
            value = row.get(field, "")
            if not isinstance(value, str) or not GRAVE_RE.search(value):
                continue
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "normalized_grave_accent_residue_in_public_paredes_fields",
                    "marker": MARKER,
