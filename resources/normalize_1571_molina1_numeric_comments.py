#!/usr/bin/env python3
"""Clear visible numeric-only comment brackets in 1571 Molina 1."""
...
PROPOSALS_PATH = Path("resources/molina1_1571_numeric_comment_proposals.tsv")
SUMMARY_PATH = Path("resources/molina1_1571_numeric_comment_summary.json")
SOURCE = "1571 Molina 1"
RAW_FIELD = "Comentario_raw_1571_molina1_numeric_comments"
MARKER = "visible_1571_molina1_numeric_comment_cleanup_2026_06_29"
QA_KEY = "qa_1571_molina1_numeric_comment_cleanup"

COMMENTARY_FIELDS = ["Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
NUMERIC_COMMENT_RE = re.compile(r"^\[([0-6])\]$")
...
def normalize_value(value: object) -> tuple[str, list[tuple[str, str]]]:
    text = str(value or "")
    if NUMERIC_COMMENT_RE.fullmatch(text):
        return "", [(text, "")]
    return text, []
...
def main() -> int:
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "cleared_visible_numeric_only_molina1_comment_brackets",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
    ...
    write_tsv(
        args.proposals,
        proposals,
        ["record_id", "original", "editado", "translation", "marker", "old_tokens", "new_tokens"],
    )
