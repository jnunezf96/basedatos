#!/usr/bin/env python3
"""Normalize narrow visible old-Spanish/Latin residue in 153? Trilingue."""
...
PROPOSALS_PATH = Path("resources/trilingue_153_oldspanish_proposals.tsv")
SUMMARY_PATH = Path("resources/trilingue_153_oldspanish_summary.json")
SOURCE = "153? Trilingüe"
RAW_FIELD = "Comentario_raw_153_trilingue_oldspanish"
MARKER = "visible_spanish_latin_153_trilingue_oldwriting_2026_06_29"
QA_KEY = "qa_153_trilingue_oldspanish"
...
REPLACEMENTS: list[tuple[re.Pattern[str], str]] = [
    (re.compile(r"(?<![\w-])baptismo(?![\w-])", re.I), "bautismo"),
    (re.compile(r"(?<![\w-])dixo(?![\w-])", re.I), "dijo"),
    (re.compile(r"(?<![\w-])moços(?![\w-])", re.I), "mozos"),
    (re.compile(r"(?<![\w-])muger(?![\w-])", re.I), "mujer"),
    (re.compile(r"(?<![\w-])quatro(?![\w-])", re.I), "cuatro"),
    (re.compile(r"(?<![\w-])rezio(?![\w-])", re.I), "recio"),
    (re.compile(r"(?<![\w-])tiêpo(?![\w-])", re.I), "tiempo"),
    (re.compile(r"(?<![\w-])vna(?![\w-])", re.I), "una"),
]
INLINE_SAFE_REPLACEMENTS: list[tuple[re.Pattern[str], str]] = [
    (re.compile(r"(?<![\w-])calçado(?![\w-])", re.I), "calzado"),
    (re.compile(r"(?<![\w-])çabullir(?![\w-])", re.I), "zabullir"),
    (re.compile(r"(?<![\w-])debaxo(?![\w-])", re.I), "debajo"),
    (re.compile(r"(?<![\w-])dixo(?![\w-])", re.I), "dijo"),
    (re.compile(r"(?<![\w-])embaçarse(?![\w-])", re.I), "embazarse"),
    (re.compile(r"(?<![\w-])enaguaçar(?![\w-])", re.I), "enaguazar"),
    (re.compile(r"(?<![\w-])muger(?![\w-])", re.I), "mujer"),
    (re.compile(r"(?<![\w-])plaça(?![\w-])", re.I), "plaza"),
    (re.compile(r"(?<![\w-])quatro(?![\w-])", re.I), "cuatro"),
]
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "normalized_narrow_visible_old_spanish_latin_residue_in_trilingue_public_text",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
