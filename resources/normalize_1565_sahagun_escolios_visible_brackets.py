#!/usr/bin/env python3
"""Remove visible square-bracket residue from 1565 Sahagun Escolios displays.

This pass handles public display leftovers that are already in normalized
commentary, not the preserved raw source packet. Bracketed apparatus numbers
become parenthesized, while lexical/gloss expansions lose only the square
brackets.
"""
...
PROPOSALS_PATH = Path("resources/sahagun_escolios_visible_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/sahagun_escolios_visible_bracket_summary.json")
SOURCE = "1565 Sahagún Escolios"
RAW_FIELD = "Comentario_raw_1565_sahagun_escolios_visible_brackets"
MARKER = "visible_1565_escolios_bracket_cleanup_2026_06_29"
QA_KEY = "qa_1565_escolios_visible_bracket_cleanup"
...
def replacement_for(inner: str) -> str:
    token = re.sub(r"\s+", " ", inner).strip()
    if not token:
        return ""
    if token.isdigit():
        return f"({token})"
...
def normalize_piece(piece: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    def replace(match: re.Match[str]) -> str:
        old = match.group(0)
        new = replacement_for(match.group(1))
        changes.append((old, new))
        return new

    new_piece = BRACKET_RE.sub(replace, piece)
    return new_piece, changes


def normalize_outside_protected(value: object) -> tuple[str, list[tuple[str, str]]]:
    text = str(value or "")
    pieces: list[str] = []
    changes: list[tuple[str, str]] = []
    last = 0
    ...
    for row in rows:
        ...
        for field in COMMENTARY_FIELDS:
            ...
            if not isinstance(value, str) or "[" not in value:
                continue
            new_value, changes = normalize_outside_protected(value)
            if new_value == value:
                continue
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "removed_visible_square_brackets_from_public_escolios_display",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
