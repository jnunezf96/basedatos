#!/usr/bin/env python3
"""Remove one-sided attached square brackets in 1551-95 Documentos commentary.

This pass handles restorations like ``[i]n``, ``termi[no]``, and
``tla[xilacalli]``. It intentionally skips standalone bracketed glosses, folios,
citations, translation alternatives, [sic], and [roto].
"""
...
PROPOSALS_PATH = Path("resources/documentos_1551_attached_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/documentos_1551_attached_bracket_summary.json")
SOURCE = "1551-95 Documentos nahuas de la Ciudad de México"
RAW_FIELD = "Comentario_raw_1551_documentos_attached_brackets"
MARKER = "visible_1551_documentos_attached_bracket_cleanup_2026_06_29"
QA_KEY = "qa_1551_documentos_attached_bracket_cleanup"

COMMENTARY_FIELDS = ["Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
BRACKET_RE = re.compile(r"\[([A-Za-zÁÉÍÓÚÜÑáéíóúüñĀĒĪŌāēīōÂÊÎÔâêîôÇç]{1,20})\]")
BR_RE = re.compile(r"<br\s*/?>", re.I)
TAG_RE = re.compile(r"<[^>]+>")
WORD_EDGE_RE = re.compile(r"[\wÁÉÍÓÚÜÑáéíóúüñÀ-ÖØ-öø-ÿĀ-ž]", re.U)
EXCLUDED_INNERS = {"sic", "roto"}
...
def normalize_value(value: object) -> tuple[str, list[tuple[str, str]], Counter[str]]:
    text = str(value or "")
    ...
    def repl(match: re.Match[str]) -> str:
        ...
        left_attached = is_word_char(before)
        right_attached = is_word_char(after)
        if left_attached == right_attached:
            return match.group(0)
        if inner.lower() in EXCLUDED_INNERS:
            skipped[f"excluded_{inner.lower()}"] += 1
            return match.group(0)
        old = match.group(0)
        changes.append((old, inner))
        return inner

    return BRACKET_RE.sub(repl, text), changes, skipped
    ...
    for row in rows:
        ...
        for field in COMMENTARY_FIELDS:
            ...
            if not isinstance(value, str) or "[" not in value:
                continue
            new_value, changes, field_skipped = normalize_value(value)
            skipped.update(field_skipped)
            if new_value == value:
                continue
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "removed_one_sided_attached_square_brackets_from_1551_documentos_commentary",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
                    "changed_token_count": len(row_changes),
                    "excluded_inners": sorted(EXCLUDED_INNERS),
