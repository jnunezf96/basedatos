#!/usr/bin/env python3
"""Normalize 1780 Clavijero grave/circumflex residue in public display fields."""
...
PROPOSALS_PATH = Path("resources/clavijero_1780_diacritic_public_proposals.tsv")
SUMMARY_PATH = Path("resources/clavijero_1780_diacritic_public_summary.json")
SOURCE = "1780 Clavijero"
MARKER = "visible_1780_clavijero_diacritic_public_2026_06_30"
QA_KEY = "qa_1780_clavijero_diacritic_public"

PUBLIC_FIELDS = ["Traducción", "Traducción (es)", "Comentario", "Comentario (es)", "Comentario_wimmer_plus_html"]
RAW_FIELD_SUFFIX = "_raw_1780_clavijero_diacritic_public"

BR_RE = re.compile(r"<br\s*/?>", re.I)
TAG_RE = re.compile(r"<[^>]+>")
TARGET_RE = re.compile("[ÀÈÌÒÙàèìòùÂÊÎÔÛâêîôû]")
WORD_RE = re.compile(r"[A-Za-zÀÈÌÒÙàèìòùÂÊÎÔÛâêîôûĀĒĪŌŪāēīōūÁÉÍÓÚÜÑáéíóúüñÇçÃÕãõ]+")
TARGET_MAP = str.maketrans("ÀÈÌÒÙàèìòùÂÊÎÔÛâêîôû", "AEIOUaeiouAEIOUaeiou")
EXACT_REPLACEMENTS = {
    "Mexìco": "México",
}
...
def normalize_text(value: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    def repl(match: re.Match[str]) -> str:
        old = match.group(0)
        new = EXACT_REPLACEMENTS.get(old, old.translate(TARGET_MAP))
        if old != new:
...
def main() -> int:
    ...
    for row in rows:
        ...
        for field in PUBLIC_FIELDS:
            value = row.get(field, "")
            if not isinstance(value, str) or not TARGET_RE.search(value):
                continue
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "normalized_grave_circumflex_residue_in_public_clavijero_fields",
                    "marker": MARKER,
