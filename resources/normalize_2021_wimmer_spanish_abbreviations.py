#!/usr/bin/env python3
"""Expand visible Spanish abbreviation/residue forms in 2021 Wimmer display text.

Most Wimmer bracket residue is Nahuatl/source-form citation data and should not
be stripped mechanically. This pass only expands a closed list of Spanish
abbreviations in public commentary fields, preserving raw commentary for rows
that are changed.
"""
...
PROPOSALS_PATH = Path("resources/wimmer_2021_spanish_abbreviation_proposals.tsv")
SUMMARY_PATH = Path("resources/wimmer_2021_spanish_abbreviation_summary.json")
SOURCE = "2021 Wimmer"
RAW_FIELD = "Comentario_raw_2021_wimmer"
MARKER = "visible_spanish_2021_wimmer_abbreviation_expansion_2026_06_29"
QA_KEY = "qa_2021_wimmer_spanish_abbreviation_expansion"
...
SPANISH_REPLACEMENTS = {
    "Ju[an]": "Juan",
    "alú[m]brar": "alumbrar",
    "ben[e]ficio": "beneficio",
    "câtaro": "cántaro",
    "có[n]las": "con las",
    "có[n]": "con",
    "d[e]dicacion": "dedicacion",
    "d[e]l": "del",
    "d[e]": "de",
    "dá[n]dose": "dándose",
    "ju[an]": "Juan",
    ...
    "ve[r]dor": "verdor",
    "vé[n]de": "vende",
    "í[n]srumento": "instrumento",
}
SPANISH_REPLACEMENT_ITEMS = sorted(SPANISH_REPLACEMENTS.items(), key=lambda item: len(item[0]), reverse=True)
STANDALONE_TOKENS = {"có[n]", "d[e]", "q[u]e", "q[ue]"}
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "expanded_visible_spanish_abbreviations_in_wimmer_display_text",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
