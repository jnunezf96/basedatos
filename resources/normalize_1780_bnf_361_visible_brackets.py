#!/usr/bin/env python3
"""Clean remaining visible square-bracket notes in 1780 ? Bnf_361 commentary."""
...
PROPOSALS_PATH = Path("resources/bnf_361_1780_visible_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/bnf_361_1780_visible_bracket_summary.json")
SOURCE = "1780 ? Bnf_361"
RAW_FIELD = "Comentario_raw_1780_bnf_361_visible_brackets"
MARKER = "visible_1780_bnf_361_bracket_cleanup_2026_06_29"
QA_KEY = "qa_1780_bnf_361_visible_bracket_cleanup"
...
INLINE_LABELS = {"s", "?", "Z"}
UNPAIRED_REPLACEMENTS = {
    "p~ipollo]": "p~ipollo)",
    "Tlacuepacholiztli.[[[": "Tlacuepacholiztli.",
}
...
def with_edge_spacing(piece: str, match: re.Match[str], replacement: str) -> str:
    before = piece[match.start() - 1] if match.start() else ""
    after = piece[match.end()] if match.end() < len(piece) else ""
    prefix = " " if is_word_char(before) and (is_word_char(replacement[:1]) or replacement.startswith("(")) else ""
    suffix = " " if is_word_char(after) and (is_word_char(replacement[-1:]) or replacement.endswith(")")) else ""
    return f"{prefix}{replacement}{suffix}"


def is_whole_piece_note(piece: str, match: re.Match[str]) -> bool:
    return not piece[: match.start()].strip() and not piece[match.end() :].strip()
...
def replacement_for_match(piece: str, match: re.Match[str]) -> str:
    ...
    before_text = piece[: match.start()].strip()
    after_text = piece[match.end() :].strip()
    if is_whole_piece_note(piece, match) or not before_text:
        replacement = inner
    elif inner in INLINE_LABELS:
        replacement = inner
    elif not after_text:
        replacement = f"({inner})"
    else:
    ...
    return with_edge_spacing(piece, match, replacement)


def tidy_piece(piece: str) -> str:
...
def normalize_piece(piece: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    for _ in range(10):
        ...
        def repl(match: re.Match[str]) -> str:
            old = match.group(0)
            new = replacement_for_match(current_piece, match)
            changes.append((old, new))
            return new

        piece, count = BRACKET_RE.subn(repl, current_piece)
        if not count or piece == current_piece:
            break
    for old, new in UNPAIRED_REPLACEMENTS.items():
        count = piece.count(old)
        if count:
            piece = piece.replace(old, new)
            changes.extend((old, new) for _ in range(count))
    if changes:
        piece = tidy_piece(piece)
...
def main() -> int:
    ...
    for row in rows:
        ...
        proposals.append(
            {
                ...
                "old_value": " | ".join(f"{field}:{old}" for field, old, _ in changed_fields),
                "new_value": " | ".join(f"{field}:{new}" for field, _, new in changed_fields),
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "cleaned_visible_square_bracket_collation_notes_outside_small_and_bold",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
    ...
    write_tsv(
        args.proposals,
        proposals,
        ["record_id", "original", "editado", "translation", "marker", "old_tokens", "new_tokens", "old_value", "new_value", "context"],
    )
