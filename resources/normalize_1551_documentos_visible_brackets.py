#!/usr/bin/env python3
"""Clean visible square-bracket display residue in 1551-95 Documentos."""
...
PROPOSALS_PATH = Path("resources/documentos_1551_visible_bracket_proposals.tsv")
SUMMARY_PATH = Path("resources/documentos_1551_visible_bracket_summary.json")
SOURCE = "1551-95 Documentos nahuas de la Ciudad de México"
RAW_FIELD = "Comentario_raw_1551_documentos_visible_brackets"
MARKER = "visible_1551_documentos_bracket_cleanup_2026_06_29"
QA_KEY = "qa_1551_documentos_visible_bracket_cleanup"
...
CROSS_TAG_REPLACEMENTS = {
    "[¿acaso apenas han nacido?": "¿acaso apenas han nacido?",
    "que se sigue].": "que se sigue).",
}
CROSS_TAG_ITEMS = sorted(CROSS_TAG_REPLACEMENTS.items(), key=lambda item: len(item[0]), reverse=True)

FOLIO_RE = re.compile(r"^F[.,]\s*\d+\s*[rv]?\s*\.?$", re.I)
EDITORIAL_RE = re.compile(
    r"^(?:"
    r"sic(?:\s+pro:.+)?|"
    r"tachado:?.*|"
    r"(?:al|del|en el|en la)?\s*margen(?:\s+(?:izquierdo|derecho))?:?.*|"
    r"entre renglones:?.*|"
    r"va entre renglones:?.*|"
    r"rúbrica|rubrica|"
    r"letra ilegible|"
    r"un renglón roto|"
    r"que no salió bien"
...
def needs_parentheses(inner: str) -> bool:
    return bool(FOLIO_RE.fullmatch(inner) or EDITORIAL_RE.fullmatch(inner))


def with_edge_spacing(piece: str, match: re.Match[str], replacement: str, compact: bool = False) -> str:
    before = piece[match.start() - 1] if match.start() else ""
    after = piece[match.end()] if match.end() < len(piece) else ""
    prefix = ""
    suffix = ""
    if not compact and is_word_char(before) and (is_word_char(replacement[:1]) or replacement.startswith("(")):
        prefix = " "
    if not compact and is_word_char(after) and (is_word_char(replacement[-1:]) or replacement.endswith(")")):
        suffix = " "
...
def replacement_for_match(piece: str, match: re.Match[str]) -> str:
    ...
    if needs_parentheses(inner):
        return with_edge_spacing(piece, match, f"({inner})")

    before = piece[match.start() - 1] if match.start() else ""
    after = piece[match.end()] if match.end() < len(piece) else ""
    compact_insert = " " not in inner and len(inner) <= 4 and (is_word_char(before) or is_word_char(after))
    return with_edge_spacing(piece, match, inner, compact=compact_insert)
    ...
    for row in rows:
        ...
        if args.apply:
            ...
            qa = {
                **qa,
                QA_KEY: {
                    "action": "cleaned_visible_square_brackets_outside_small_and_bold",
                    "marker": MARKER,
                    "raw_field": RAW_FIELD,
                    "raw_preserved": True,
