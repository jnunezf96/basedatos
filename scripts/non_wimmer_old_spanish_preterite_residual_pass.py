...
REPORT_PATH = ROOT / "scripts" / "non_wimmer_old_spanish_preterite_residual_report.jsonl"


RESIDUAL_PRETERITE_TAIL_RE = re.compile(
    r"(?:(?<=\.)\s*|(?<=\s))\b(?:pr|preteri|preter)\s*:\s*.*$",
    re.I,
)
...
def clean(value: str, source: str) -> tuple[str, list[str]]:
    ...
    new = RESIDUAL_PRETERITE_TAIL_RE.sub("", text)
    reasons: list[str] = []
    if new != text:
        reasons.append("molina_residual_preterite_tail")
