...
REPORT_PATH = ROOT / "scripts" / "non_wimmer_old_spanish_assi_report.jsonl"


ASSI_RE = re.compile(r"\b(?:assi|anssi|assí|ansí)\b", re.I)
...
def clean(value: str) -> tuple[str, list[str]]:
    ...
    def replace_match(match: re.Match[str]) -> str:
        token = match.group(0)
        changed_forms.append(token.lower())
        return replacement_for(token)

    new = ASSI_RE.sub(replace_match, text)
    reasons: list[str] = []
    if new != text:
        reasons.append("assi_token")
        reasons.extend(f"token:{token}" for token in sorted(set(changed_forms)))
